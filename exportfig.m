...
%   EXPORTFIG(H, FILENAME, KEEP_BG_COLOR) writes the figure H to 
%   FILENAME.  H is a figure handle and FILENAME is a string that 
%   specifies the name of the output file. KEEP_BG_COLOR is a flag
%   that (if set to 1) keeps the current figure color, otherwise,
%   it sets the figure color to white.
%
...
%
%   Examples:
%     exportfig(gcf,0,'fig1.eps','height',3);
...
%       ratio on screen. Sets background color to white.
%
%     exportfig(gcf,1,'fig2.eps', 'FontMode', 'fixed',...
...
%       the figure's PaperPostion width and height. Sets background
...
% convert fonts to Helvetica
% set(get(get(H,'CurrentAxes'),'XLabel'),'FontName','Helvetica');
% set(get(get(H,'CurrentAxes'),'YLabel'),'FontName','Helvetica');
% set(get(get(H,'CurrentAxes'),'ZLabel'),'FontName','Helvetica');
% set(get(get(H,'CurrentAxes'),'Title'),'FontName','Helvetica');
% set(get(H,'CurrentAxes'),'FontName','Helvetica');

paramPairs = varargin(4:end);
